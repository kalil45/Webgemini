...
function CapitalContent({ refreshDashboardData }) {
  ...
  const fetchTotalCapital = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setCurrentCapital(data.totalCapital);
      } else {
  ...
  const handleAddCapital = async () => {
    if (amountToAdd && !isNaN(amountToAdd) && parseFloat(amountToAdd) > 0) {
      try {
        const response = await fetch('http://localhost:5000/api/capital', {
          method: 'POST',
          headers: {
  ...
  return (
    <div className="card shadow-sm mb-4">
      ...
      <div className="card-body">
        ...
        <p className="card-text fs-3">Rp {currentCapital.toLocaleString()}</p>
